 ...
 */
public class SessionDataCarsByRelativeClass extends SessionData {
    ...
    public SessionDataCarsByRelativeClass(iRacingSIMPlugin SIMPlugin, SessionDataCars cars) {
        super(SIMPlugin, "CarsByRelativeClass", -1, "integer",State.OFF);
    ...
    private void _update() {
        if (this._needsUpdating()) {
            ...
            String refClass = m_SIMPlugin.getSession().getCar("REFERENCE").getClassName().getString();
...
//                &&  !c.getIsSpectator().getBoolean()
                ...
                &&  c.getClassName().getString().equals(refClass)
                ) {
