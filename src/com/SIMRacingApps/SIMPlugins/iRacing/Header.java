...
public class Header {
	...
	private SubHeader  m_subheader;		//for IBT files
	...
	public SubHeader getSubHeader() {
		if (m_subheader == null) {
			byte subheader_buffer[] = new byte[SubHeader.SIZEOF_SUBHEADER];
			m_subheader = new SubHeader(ByteBuffer.wrap(subheader_buffer));
		}
		return m_subheader;
	}
	public Header getSubHeader( SubHeader subheader) {
		m_subheader = subheader;
		return this;
	}

	public int getVer()	 				{ return m_bytebuffer.getInt(0);  }
	public int getStatus() 				{ return m_bytebuffer.getInt(4);  }
	public int getTickRate() 			{ return m_bytebuffer.getInt(8);  }
	public int getSessionInfoUpdate()	{ return m_bytebuffer.getInt(12); }
	public int getSessionInfoLen() 		{ return m_bytebuffer.getInt(16); }
	public int getSessionInfoOffset()  	{ return m_bytebuffer.getInt(20); }
	public int getNumVars() 			{ return m_bytebuffer.getInt(24); }
	public int getVarHeaderOffset() 	{ return m_bytebuffer.getInt(28); }
	public int getNumBuf() 				{ return m_bytebuffer.getInt(32); }
	public int getBufLen() 				{ return m_bytebuffer.getInt(36); }
	...
	public boolean isValid() {
		if (
			getVer() == 0
		||	getTickRate() == 0
		||	(getSessionInfoUpdate() > 0 && (getSessionInfoLen() == 0 || getSessionInfoOffset() == 0))
		||	getNumVars() == 0
...
//	    #iRacing gives us 16ms to read it from the time the timer lets us know it's there.
		...
		if (header != null
		&& (
				getVarBuf_TickCount(getLatest_VarBuf()) != header.getVarBuf_TickCount(getLatest_VarBuf())
				||
	...
	public String toJSON() {
        ...
        for (int i=0; i < this.getNumBuf(); i++) {
            json.append(String.format("    { \"tickcount\": %d, \"bufOffset\": %d }",
				this.getVarBuf_TickCount(i),
        ...
        if (this.getSubHeader().getSessionStartDate() != 0) {
			json.append("},\n   \"subheader\": {");
			json.append(String.format("  \"size\": %d,\n",SubHeader.SIZEOF_SUBHEADER));
			json.append(String.format("  \"sessionStartDate\": %d,\n",this.getSubHeader().getSessionStartDate())); //POSIX::strftime("%c",localtime($subheader->SessionStartDate));
			json.append(String.format("  \"sessionStartTime\": %f,\n",this.getSubHeader().getSessionStartTime()));
			json.append(String.format("  \"sessionEndTime\": %f,\n",this.getSubHeader().getSessionEndTime()));
			json.append(String.format("  \"sessionLapCount\": %d,\n",this.getSubHeader().getSessionLapCount()));
            json.append(String.format("  \"sessionRecordCount\": %d\n",this.getSubHeader().getSessionRecordCount()));
			json.append("}\n");
        }
		else {
			json.append("}\n");
